...
export default function ShiftScheduleApp() {
  ...
  const getDayOfWeekInIsrael = (dateString) =>
    createUtcDateFromIso(dateString).getUTCDay();
  ...
  const shifts = {
    morning: {
      name: 'משמרת בוקר',
      time: 'תדריך 06:30 | תחילת משמרת 07:00',
      icon: '🌅',
    },
    noon: {
      name: 'משמרת צהריים',
      time: 'תדריך 13:30 | תחילת משמרת 14:00',
      icon: '☀️',
    },
    noonLate: {
      name: 'משמרת צהריים',
      time: 'תדריך 14:30 | תחילת משמרת 15:00',
      icon: '☀️',
    },
    night: {
      name: 'משמרת לילה',
      time: 'תדריך 20:30 | תחילת משמרת 21:00',
      icon: '🌙',
    },
  };
  ...
  const getUnitOnShift = (date, shift) => {
    ...
    const totalDays =
      daysDiff >= 0
        ? daysDiff
        : daysDiff + Math.ceil(Math.abs(daysDiff) / 21) * 21;
  ...
  const generateWeeklySchedule = (weekStartDate) => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      schedule.push({
        day: HEBREW_DAYS[i],
        date: dateStr,
        dateDisplay: createDisplayDateForIsrael(dateStr).toLocaleDateString(
          'he-IL',
          {
            timeZone: ISRAEL_TIME_ZONE,
            day: 'numeric',
          ...
          <div className='bg-white rounded-2xl shadow-xl p-6 md:p-8'>
            <div className='mb-6'>
              ...
              <p className='text-sm text-gray-500 mt-2'>
                {formatDate(selectedDate)} • יום{' '}
                {getDayOfWeekHebrew(selectedDate)}
              </p>
            </div>
              ...
              <table className='w-full border-collapse'>
                <thead>
                  <tr className='bg-gray-50'>
                    <th className='p-3 text-right font-semibold text-gray-700 border-b-2 border-gray-200 sticky right-0 bg-gray-50'>
                      משמרת
                    </th>
                    {weeklySchedule.map((daySchedule, index) => (
                      <th
                        key={index}
                        className='p-3 text-center font-semibold text-gray-700 border-b-2 border-gray-200 min-w-[100px]'
                      >
                        <div>{daySchedule.day}</div>
                        <div className='text-xs font-normal text-gray-500'>
                          {daySchedule.dateDisplay}
                        </div>
                      </th>
                    ))}
                  </tr>
                      ...
                      >
                        <span
                          className={`inline-block px-4 py-2 rounded-full font-bold text-white ${
                            daySchedule.morning === 1
                              ? 'bg-blue-500'
                              : daySchedule.morning === 2
                                ? 'bg-green-500'
                                : 'bg-purple-500'
                          }`}
                        >
